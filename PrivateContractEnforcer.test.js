import { increaseTimeTo, duration } from './helpers/increaseTime';
import latestTime from './helpers/latestTime';
import ether from './helpers/ether';

const BigNumber = web3.BigNumber;

require('chai')
  .use(require('chai-as-promised'))
  .use(require('chai-bignumber')(BigNumber))
  .should();

var ContractChannel = artifacts.require('../contracts/channels/PrivateContractEnforcer.sol');
var ECRecoveryLib = artifacts.require('../contracts/ECRecovery.sol');

contract('PrivateContractEnforcer', function (accounts) {
  let contractChannel;
  let ecrecovery;
  // eslint-disable-next-line max-len
  const etherBetBytecode = '';
  const value = ether(42);

  before(async function () {
    const ecRecoveryLib = await ECRecoveryLib.new();
    ContractChannel.link('ECRecovery', ecRecoveryLib.address);
    contractChannel = await ContractChannel.new();
  });

  it('recover using web3.eth.sign()', async function () {
    //signed with account[0];
    const metamaskEtherBet = '0x84dbd70d0aea5bb1e416ae825f808b8149fd0f363e953ba3c4a1cf0f79ca3ecb547ee5f81d009142b0cc82a4fbf4608d77bd2106a32923966cfcbdac2e8b062b1c';

    await contractChannel.sendTransaction({value: value, from: accounts[0]});
    await contractChannel.sendTransaction({value: value, from: accounts[1]});
    
    ether(84).should.be.bignumber.equal(web3.eth.getBalance(contractChannel.address));
    const currentBalance = web3.eth.getBalance(accounts[0]);
    let settlingTime = latestTime() + duration.weeks(5);
    await increaseTimeTo(settlingTime);
    await contractChannel.executeContract(web3.sha3(etherBetBytecode, {encoding: 'hex'}), metamaskEtherBet, etherBetBytecode, {from: accounts[1]});
    const afterBalance = web3.eth.getBalance(accounts[0]);
    
    afterBalance.should.be.bignumber.greaterThan(currentBalance);
  });
});
